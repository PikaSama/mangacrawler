...
// 本地模块
import { WorkerDownloadParam, WorkerCallbackFn, OutTimer, prepare } from '../modules/misc';
    ...
    getUrl(
        ...
        (err, num): void => {
            if (err) {
                ...
                console.error(
                    `${chalk.whiteBright.bgRed(' Error ')} Oops! Something went wrong, try again? [M-0x0001]`,
                );
                process.exit(1);
            } else {
                timer.clear();
...
function resolveImages(): void {
    ...
    for (let i = 2; i < mangaImages + 1; i++) {
        // 错误时，结束进程
        resolveUrl.push(
            { url: `${mangaUrl.slice(0, mangaUrl.length - 5)}-${i}.html` },
            (err: Error, num: number): void => {
                if (err) {
                    console.error(`\n\n${chalk.whiteBright.bgRed(' Error ')} ${err} \n`);
                    console.error(
                        `${chalk.whiteBright.bgRed(' Error ')} Oops! Something went wrong, try again? [M-0x0101]`,
                    );
                    process.exit(1);
                } else {
                    resolvedImgs += num;
...
function downloadImages(): void {
    ...
    // 下载图片(并发控制)
    const download: async.QueueObject<object> = async.queue(
        ({ url }: WorkerDownloadParam, callback: WorkerCallbackFn): void => {
        ...
        if (crawlList.hasOwnProperty(i)) {
            ...
            download.push({ url: crawlList[i] }, (err: Error, result: number): void => {
                if (err) {
                    console.error(`\n\n${chalk.whiteBright.bgRed(' Error ')} ${err} \n`);
                    console.error(
                        `${chalk.whiteBright.bgRed(' Error ')} Oops! Something went wrong, try again? [M-0x0201]`,
                    );
                    process.exit(1);
                } else {
