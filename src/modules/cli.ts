        ...
        inquirer.prompt([
            {
                ...
                validate(val: string): string | boolean {
                    ...
                    // 判断url包含的斜杠，仅需3个
                    if (blocks.length !== 4) {
                        return Logger.errStr('Invalid URL format. [I-0x0101]');
                    }
                    // 判断协议是否合法
                    else if (blocks[0].match("http:") || blocks[0].match("https:")) {
                        // 判断第二个块是否无值
                        if (!(blocks[1] === '')) {
                            return Logger.errStr('Invalid URL usage. [I-0x0103]');
                        }
                        // 判断网站是否正确 && 是否以'm'开头 && 是否包含分页符号“-”
                            ...
                            return Logger.errStr('Invalid domain or Manga ID. [I-0x0104]');
                        }
                    }
                    else {
                        return Logger.errStr('Unsupported transport protocol. [I-0x0102]');
                    }
                },
            },
        ]).then(({ url }) => {
        ...
        inquirer.prompt([
            {
                ...
                validate(val:string): string | boolean {
                    ...
                    // 判断url包含的斜杠，仅需5个
                    if (blocks.length !== 6) {
                        return Logger.errStr('Invalid URL format. [I-0x0201]');
                    }
                    // 判断协议是否合法
                    else if (blocks[0].match("http:") || blocks[0].match("https:")) {
                        // 判断第二个块是否有值 || 第五个块是否无值
                        if (!(blocks[1] === '') || blocks[4] === '') {
                            return Logger.errStr('Invalid URL usage. [I-0x0203]');
                        }
                        // 判断网站是否正确 && 目录是否指向“manhua” && 后缀是否为“.html”
                            ...
                            return Logger.errStr('Invalid domain or resource directory or Manga ID. [I-0x0204]');
                        }
                    }
                    else {
                        return Logger.errStr('Unsupported transport protocol. [I-0x0202]');
                    }
                },
            },
        ]).then(({ url }) => {
            ...
            // 保存路径
            {
                ...
                validate(val: string): string | boolean {
                    ...
                    if (val.slice(-1) === "/" && val.length > 1) {
                        return Logger.errStr('You don\'t need to add "/" at the end of the path. [I-0x0001]');
                    }
                    else {
                        return true;
                    }
            ...
            // 下载请求限制
            {
                ...
                validate(val: number): string | boolean {
                    ...
                    if (val >=1 && val <= 16) {
                        ...
                        return Logger.errStr('Invalid number. [I-0x0002]');
                    }
                },
